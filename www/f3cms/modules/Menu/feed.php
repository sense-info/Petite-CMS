...
class fMenu extends Feed
{
    ...
    public static function get_menus($parent_id = -1, $force = 0)
    {
        ...
        $rows = mh()->query('SELECT c.id, l1.title, c.uri, c.theme, c.blank, l1.badge, c.color, c.icon, c.parent_id, l2.title AS parent FROM `' . self::fmTbl() . '` c ' . $join . $condition . ' ORDER BY c.sorter, c.id ')->fetchAll(\PDO::FETCH_ASSOC);
